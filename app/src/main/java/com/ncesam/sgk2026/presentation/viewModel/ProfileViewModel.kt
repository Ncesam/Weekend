    ...
    suspend fun onEvent(event: ProfileEvent) {
        when (event) {
           is ProfileEvent.UploadAvatar -> {
               ...
               }.onSuccess { user -> _state.update { state ->  state.copy(avatar = user.avatar, firstName = user.firstName, email = user.email, dateInfo = formatDate(
                   validateDate(user.born) ?: return@onSuccess
               ), gender = user.gender.slice((0..3)), notificationActive = userRepository.getNotificationActive()) } }
           }
